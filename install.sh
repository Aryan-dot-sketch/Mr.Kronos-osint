...
function Preference {
	...
	read -p"$GREEN[#MR.KRONOS#]$WHITE-->" Language
	while [ "$Language" == "" ];
		do
		printf "${WHITE}\nSELECT YOUR GUI-DEFAULT LANGUAGE\n(1)ENGLISH\n(2)ITALIANO\n(3)FRANÇAIS\n(4)BROWSER-LANGUAGE\n\n"
		read -p"$GREEN[#MR.KRONOS#]$WHITE-->" Language
	...
	read -p"$GREEN[#MR.KRONOS#]$WHITE-->" Theme
	while [ "$Language" == "" ];
		do
		printf "${WHITE}\nSELECT YOUR GUI-DEFAULT THEME\n(1)LIGHT\n(2)DARK\n(3)HIGH-CONTRAST\n\n"
		read -p"$GREEN[#MR.KRONOS#]$WHITE-->" Language
...
function Mail_Options {
	...
	read -p"$GREEN[#MR.KRONOS#]$WHITE-->" Opt
	while [ "$Opt" = "" ];
		do
        printf "${BLUE}\n[?]${WHITE}WOULD YOU LIKE TO ENABLE EMAIL-OPTION(1)YES(2)NO\n\n"
		read -p"$GREEN[#MR.KRONOS#]$WHITE-->" Opt
	...
	if [ $Opt == 1 ];
		...
		read -p"$GREEN[#MR.KRONOS#]$WHITE-->" recipient
		while [ "$recipient" = "" ];
			do
			printf "${WHITE}\nINSERT YOUR RECIPIENT EMAIL\n\n"
			read -p"$GREEN[#MR.KRONOS#]$WHITE-->" recipient
		...
		read -sp"$GREEN[#MR.KRONOS#]$WHITE-->" password
		while [ "$password" = "" ];
			do
			printf "${WHITE}\nINSERT YOUR EMAIL PASSWORD\n\n"
			read -sp"$GREEN[#MR.KRONOS#]$WHITE-->" password
		...
		read -p"$GREEN[#MR.KRONOS#]$WHITE-->" destination
		while [ "$destination" = "" ];
			do
			printf "${WHITE}\nINSERT YOUR DESTINATION EMAIL\n\n"
			read -p"$GREEN[#MR.KRONOS#]$WHITE-->" destination
		...
		read -p"$GREEN[#MR.KRONOS#]$WHITE-->" server
		while [ "$server" = "" ];
			do
			printf "${WHITE}\nINSERT YOUR SMTP SERVER EX smtp.test.com\n\n"
			read -p"$GREEN[#MR.KRONOS#]$WHITE-->" server
		...
		read -p"$GREEN[#MR.KRONOS#]$WHITE-->" port
		while [ "$port" = "" ];
			do
...
function Options {
	...
	read -sp"$GREEN[#MR.KRONOS#]$WHITE-->" up_pass
	while [ "$up_pass" = "" ];
		do
        printf "${WHITE}\nINSERT YOUR UPDATE-PASSWORD \n\n"
        read -sp"$GREEN[#MR.KRONOS#]$WHITE-->" up_pass
	...
	read -p"$GREEN[#MR.KRONOS#]$WHITE-->" Token
	while [ "$Token" == "" ];
		do
		printf "${WHITE}\n\nWOULD YOU LIKE TO ACCESS YOUR DATABASE ON OTHER DEVICES(ON THE SAME NETWORK)?(1)YES(2)NO\n\n"
		read -p"$GREEN[#MR.KRONOS#]$WHITE-->" Token
	...
	read -p"$GREEN[#MR.KRONOS#]$WHITE-->" Access
	while [ "$Access" == "" ];
		do
		printf "${WHITE}\n\nWOULD YOU LIKE TO ADD SOME CREDENTIALS FOR ACCESS THE DATABASE?(1)YES(2)NO\n\n"
		read -p"$GREEN[#MR.KRONOS#]$WHITE-->" Access
	...
	read -p"$GREEN[#MR.KRONOS#]$WHITE-->" DateFormat
	while [ "$DateFormat" == "" ];
		do
		printf "${WHITE}\nSELECT YOUR DATE-FORMAT\n(1)EUROPE(DD/MM/YYYY)\n(2)AMERICA'USA'(MM/DD/YYYY)\n(3)ASIA(YYYY/MM/DD)\n\n"
		read -p"$GREEN[#MR.KRONOS#]$WHITE-->" DateFormat
	...
	cd Configuration
	echo ";THIS FILE HAS BEEN GENERATE BY MR.KRONOS INSTALLER">Configuration.ini
...
echo '{
	...
	echo ";THIS FILE HAS BEEN GENERATE BY MR.KRONOS INSTALLER">Configuration/Configuration.ini
	...
	echo "password = Kronos">>Configuration/Configuration.ini
	...
	printf "\n${WHITE}UPDATE-PASSWORD:${GREEN}Kronos\n"
...
function installer {
	...
	if [ $confvar == 1 ];
		...
		read -p "$GREEN[#MR.KRONOS#]$WHITE-->" selected
		while [ "$selected" == "" ];
			do
			printf "${BLUE}\n\nWOULD YOU LIKE TO SET(1)MANUAL-INSTALLATION(2)AUTO-INSTALLATION\n\n"
			read -p "$GREEN[#MR.KRONOS#]$WHITE-->" selected
		...
		printf "${LIGHTGREEN}\n\nTHANK YOU FOR HAVE INSTALLED Mr.Kronos\n\n"
		exit 0
	fi
