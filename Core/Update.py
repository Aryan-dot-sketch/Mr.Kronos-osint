...
class Downloader:
    ...
    def Update():
        ...
        os.chdir(Path)
        if os.path.exists("Mr.Kronos_Old"):
            shutil.rmtree("Mr.Kronos_Old",)
        if os.name == "nt":
            os.system("rename Mr.Kronos Mr.Kronos_Old 2>NUL >NUL")
        else:
            os.system("mv Mr.Kronos Mr.Kronos_Old &>/dev/null")
        os.system("git clone https://github.com/aryanhubhaimai/Mr.Kronos-OSINT")
        choice = int(input(Font.Color.BLUE + "\n[+]" + Font.Color.WHITE +
                     Language.Translation.Translate_Language(filename, "Update", "Choice", "None")))
        if choice == 1:
            os.remove("Mr.Kronos_Old")
